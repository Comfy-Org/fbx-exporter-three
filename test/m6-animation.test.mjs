// M6: animation round-trip via FBXLoader.
//
// Run: node test/m6-animation.test.mjs
...
// ============================================================================
// CA. Position-only translation animation
// ============================================================================
...
test('CA1: Mesh translation track round-trips with the same times and values', () => {
  ...
  // Translate from (0,0,0) to (5,0,0) over 1 second.
  ...
  // FBXLoader returns a Group when there's exactly one child; check animations.
  ...
  // First value should be (0,0,0), last should be (5,0,0)
...
test('CB1: Quaternion rotation track survives quaternion → Euler → quaternion round-trip', () => {
  ...
  // Spin 90° around Y over 1 second.
  ...
  // Last keyframe should match q1 (within FP tolerance, allowing for the
  // double pass through Euler).
...
// ============================================================================
// CD. Multiple tracks on the same object
// ============================================================================
...
// ============================================================================
// CE. Animating a Bone inside a SkinnedMesh
// ============================================================================

test('CE1: SkinnedMesh bone animation round-trips', () => {
  const scene = new THREE.Scene();
  // Reuse the simple skinned scene construction.
...
// ============================================================================
// CF. Multiple clips on the same scene
// ============================================================================
...
test('CG1: Scene with no animations exports cleanly (no AnimStack nodes)', () => {
  ...
  const bytes = new FBXExporter().parseSync(scene);
  // Verify no AnimationStack in the byte stream.

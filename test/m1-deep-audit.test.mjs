...
test('AA1: FBX property type tag byte values match exactly', () => {
...
test('AA2: ARRAY_COMPRESS_THRESHOLD = 128', () => {
...
test('AB7: writeBytes accepts a Uint8Array view with non-zero byteOffset', () => {
  ...
  const view = new Uint8Array(big.buffer, 2, 4);
  const bw = new BinaryWriter(16);
  bw.writeBytes(view);
...
test('AC6: addBytes pack("<I", len) + data', () => {
  const e = new FBXElem('X');
...
test('AD1: array exactly at 128 bytes uses encoding=0', () => {
  const e = new FBXElem('X');
  e.addInt32Array(new Int32Array(32));
...
test('AD2: array at 129 bytes uses encoding=1 (zlib)', () => {
  const e = new FBXElem('X');
  e.addInt32Array(new Int32Array(33));
  const payload = e.propsData[0];
  const dv = new DataView(payload.buffer);
...
test('AF1: header signature exactly matches _HEAD_MAGIC', () => {
...
test('AG1: time-hack replaces FileId 16 zeros with the canonical 16-byte FILE_ID', () => {
  ...
  fileId.addBytes(new Uint8Array(16));

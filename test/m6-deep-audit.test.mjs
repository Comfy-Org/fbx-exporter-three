// M6 deep audit — byte-level invariants for animation nodes.
//
// Run: node test/m6-deep-audit.test.mjs
...
test('CAA1: secondsToKTime(1.0) = FBX_KTIME (= 46186158000)', () => {
  ...
  // P record: name, type, subtype, flags, value (int64).
...
test('CAA3: KeyTime array entries are exact multiples of FBX_KTIME/1s for integer times', () => {
  // Times [0, 0.5, 1.0] → KTime [0, FBX_KTIME/2, FBX_KTIME].
  const { tree } = exportTree(buildPositionScene());
  const objects = findRoot(tree, 'Objects');
  ...
  // int64 array: 3 entries × 8 bytes = 24 bytes.
...
// ============================================================================
// CAC. AnimationCurveNode structure
// ============================================================================
...
test('CAC2: AnimationCurveNode attrName encodes the kind ("T" for translation)', () => {
  const { tree } = exportTree(buildPositionScene());
  const curveNode = findChild(findRoot(tree, 'Objects'), 'AnimationCurveNode');
  // attrName = "T\x00\x01AnimCurveNode" for position.
  assert.ok(curveNode.props[1].startsWith('T\x00\x01'),
    `attrName: ${JSON.stringify(curveNode.props[1])}`);
});

// ============================================================================
// CAD. AnimationCurve structure + child order
// ============================================================================

test('CAD1: AnimationCurve child order matches Blender exactly', () => {
  // Blender (lines 2116-2122):
  //   Default, KeyVer, KeyTime, KeyValueFloat, KeyAttrFlags,
  //   KeyAttrDataFloat, KeyAttrRefCount
  const { tree } = exportTree(buildPositionScene());
...
test('CAD3: KeyTime is INT64 array; KeyValueFloat is FLOAT32 array (not 64!)', () => {
  // Blender (lines 2118-2119):
  //   KeyTime = int64 (high precision time)
  //   KeyValueFloat = float32 (single precision values — note the asymmetry)
  const { tree } = exportTree(buildPositionScene());
  const curve = findChildren(findRoot(tree, 'Objects'), 'AnimationCurve')[0];
...
// ============================================================================
// CAF. Quaternion bake — unrolling and degrees
// ============================================================================
...
test('CAF1: Quaternion track is baked to 3 axes as Float32 degrees', () => {
  ...
  const curves = findChildren(findRoot(tree, 'Objects'), 'AnimationCurve');
  // Find the Y-axis curve via OP relationship.
  const curveNode = findChild(findRoot(tree, 'Objects'), 'AnimationCurveNode');
  const conns = parseConnections(tree);
  let yCurveUid = null;
...
test('CAF2: Quaternion unrolling — adjacent keys never differ by >180° on any axis', () => {
  // Spin 720° around Y — naive Euler would wrap and the curve would have
  // 360° discontinuities. With unrolling, consecutive degree values should
  // change monotonically by ≤ a sensible delta.
  const scene = new THREE.Scene();
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(), new THREE.MeshStandardMaterial());
  mesh.name = 'R2';
  scene.add(mesh);
  // 9 keyframes: 0°, 90°, 180°, 270°, 360°, 450°, 540°, 630°, 720° around Y.
  ...
  // Adjacent values must differ by ≤ 180°. Without unrolling, the third
  // keyframe wraps from 90 to -90 (a 180° flip in the naive Euler),
  // OR the next wraps at the antipode. With unrolling, the sequence
  // climbs monotonically through 0, 45, 90, 135, 180, 225, ...
  // (or a continuous decreasing alternative).
...
// ============================================================================
// CAG. Definitions registers the 4 animation templates
// ============================================================================
...
test('CAG2: AnimationCurve count = stacks × tracks × 3 axes', () => {
  // 1 stack × 1 track (position) × 3 axes = 3 AnimationCurve users.
  const { tree } = exportTree(buildPositionScene());
  const defs = findRoot(tree, 'Definitions');

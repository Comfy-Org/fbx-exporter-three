...
test('FB1: DecayStart is a p_double', () => {
  const scene = new THREE.Scene();
  scene.add(new THREE.PointLight());
  const attr = getAttr(exportToTree(scene).tree, 'Light');
...
test('FB4: DecayType=2 (INVERSE_SQUARE)', () => {
  const scene = new THREE.Scene();
  scene.add(new THREE.PointLight());
  const attr = getAttr(exportToTree(scene).tree, 'Light');
  const p70 = findChild(attr, 'Properties70');
...
test('FC2: Camera trailing fields in order (TypeFlags → ... → CameraOrthoZoom)', () => {
  const scene = new THREE.Scene();
  scene.add(new THREE.PerspectiveCamera());
  const attr = getAttr(exportToTree(scene).tree, 'Camera');
  let p70Seen = false;
...
test('FC5: FieldOfView uses p_fov type (matches p_fov, line 691)', () => {
  const scene = new THREE.Scene();
  scene.add(new THREE.PerspectiveCamera());
  const attr = getAttr(exportToTree(scene).tree, 'Camera');
  const p70 = findChild(attr, 'Properties70');
...
test('FE1: Light NodeAttribute → Light Model direction matches (src=attr, dst=model)', () => {
  const scene = new THREE.Scene();
  scene.add(new THREE.PointLight());

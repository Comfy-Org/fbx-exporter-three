// M8: Light + Camera round-trip via FBXLoader.
//
// Run: node test/m8-light-camera.test.mjs

import { strict as assert } from 'node:assert';
import { unzlibSync } from 'fflate';

globalThis.self = globalThis;
// FBXLoader.js:1069 unconditionally reads window.innerWidth even when
// AspectWidth/Height are present in the camera attribute. Stub for Node.
...
test('EC4: Light Color matches three.js .color (linear, sRGB conversion is FBXLoader-side)', () => {
  ...
  // Color P record: props 4,5,6 = r,g,b. We wrote three.js color directly (linear).
...
test('EC5: SpotLight OuterAngle = degrees(angle), InnerAngle = OuterAngle × (1 - penumbra)', () => {
  // Blender formula matches Blender export_fbx_bin.py:625-626 inverse.
  const scene = new THREE.Scene();
...
test('EC9: Camera TypeFlags="Camera" + GeometryVersion=124 trailing fields', () => {
  // Blender writes these as top-level children of NodeAttribute, not in P70.
  const scene = new THREE.Scene();
...
// ============================================================================
// EE. Light/Camera position survives the round-trip via Model Lcl Translation
// ============================================================================
...
test('EG1: Definitions NodeAttribute Count includes lights + cameras + nulls + bones', () => {
  ...
  scene.add(new THREE.DirectionalLight());
  scene.add(new THREE.PerspectiveCamera());
  scene.add(new THREE.Group());  // 1 Null
  const { tree } = exportToTree(scene);
  ...
  // 2 lights + 1 camera + 1 null = 4 base NodeAttribute users.
  // + 1 auxiliary Null NodeAttribute for the DirectionalLight's off-scene
  // target (M8 follow-up: LookAtProperty round-trip via FBXLoader).

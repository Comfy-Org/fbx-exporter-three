// E2E #4: parameterized round-trip across a sampling of three.js's official
// example assets at H:/three.js/examples/models. Each one stresses a
// different milestone:
//
//   duck.glb           — baseline single-mesh w/ texture
//   Horse.glb          — skinned + 1 animation clip (small)
//   Soldier.glb        — skinned + 3 named animation clips
//   Flamingo.glb       — morph-target animation (no skinning)
//   facecap.glb        — heavy morph targets (face capture)
//   LittlestTokyo.glb  — large multi-mesh scene (~stress test)
//
// We deliberately use .glb (binary glTF) so textures + buffers are
// self-contained — no external-file I/O required for the loader.
//
// Run: node test/e2e-4-threejs-models.test.mjs
...
// Stub document so FBXLoader's TextureLoader/ImageLoader path doesn't crash.
// The fake <img> element fires `load` on src set — no actual decode happens
// (we have no image runtime in Node) but Texture objects and metadata
// survive, which is what our structural checks need.
globalThis.document = {
...
// Skipped: duck.glb / LittlestTokyo.glb (Draco-compressed — need DRACOLoader)
//          facecap.glb (KTX2-compressed textures — need KTX2Loader)
// Those failures are GLTFLoader limitations, not ours.
const TARGETS = [
...
// ---------------------------------------------------------------------------
// One model run — returns a Result row.
// ---------------------------------------------------------------------------
...
// Exit non-zero if any model fully failed (LOAD/EXPORT/IMPORT-FAIL or PARTIAL).

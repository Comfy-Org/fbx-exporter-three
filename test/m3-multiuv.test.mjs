// M3 follow-up: multi-UV-layer round-trip. three.js's standard convention
// for lightmaps / AO is a second UV set under attribute name `uv1` (modern
// three.js, formerly `uv2`). FBX represents extra UV sets as additional
// `LayerElementUV` nodes inside Geometry, each with its own TypedIndex.
//
// Run: node test/m3-multiuv.test.mjs
...
// inline FBX parser (minimal — extract UV data)
...
function buildMeshWithUvs(uvSets) {
  // 4-vertex quad (two triangles) so we have distinguishable UVs.
...
test('A1: only `uv` attribute → exactly 1 LayerElementUV', () => {
  ...
  assert.equal(uvLayers[0].props[0], 0);  // TypedIndex
});

// =====================================================================
// B. Two UV layers (uv + uv1) — modern three.js for aoMap/lightMap
// =====================================================================

test('B1: uv + uv1 → 2 LayerElementUV nodes with distinct TypedIndex', () => {
  const scene = new THREE.Scene();
  scene.add(buildMeshWithUvs({
    uv:  [0, 0, 1, 0, 1, 1, 0, 1],          // primary tex coords
    uv1: [0.1, 0.1, 0.9, 0.1, 0.9, 0.9, 0.1, 0.9],  // lightmap coords
  }));
  const { tree } = exportToTree(scene);
  const geo = findRoot(tree, 'Objects').children.find((c) => c.name === 'Geometry');
  const uvLayers = findChildren(geo, 'LayerElementUV');
...
test('B3: Layer entries — Layer 0 lists uv0, Layer 1 lists uv1', () => {
  ...
  // Each Layer has child `LayerElement` records of (Type, TypedIndex).
...
// =====================================================================
// C. Four UV layers (uv / uv1 / uv2 / uv3) — exotic but supported
// =====================================================================
...
// =====================================================================
// D. Round-trip through FBXLoader — does it expose multiple UV sets?
// =====================================================================
...
test('D1: FBXLoader re-imports uv1 as attributes.uv1 (modern naming)', async () => {
  ...
  // FBXLoader naming convention varies by version. Just check that AT LEAST
  // one extra UV-like attribute is present on the re-imported geometry.
  ...
  // Spot-check: the secondary UV values should differ from the primary.
  const primary = attrs.uv ?? attrs.uv0;
  const secondary = attrs.uv1 ?? attrs.uv2;
  assert.ok(primary && secondary, 'both UV sets re-imported');
  // The values should NOT be identical (we wrote distinct data).
  let differs = false;

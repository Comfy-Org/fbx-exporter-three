...
FBX binary exporter for three.js scenes. Writes the same FBX 7.4 dialect as Blender's `io_scene_fbx` addon — Unity / Unreal / Maya / Blender / three.js's own `FBXLoader` all consume the output cleanly.
...
- **Verified**: 446 unit tests + Blender oracle parity (structural diff against `bl_cube.fbx` / `bl_cube_skinned.fbx`) + 10-asset real-world E2E (raw OBJ, Mixamo glTFs, textured OBJ, etc.).
...
| `customProperties` | `boolean` | `false` | Emit each Object3D's `userData` as user-defined Properties70 entries (`U` flag, à la Blender's `use_custom_props`). Maps booleans → `p_bool`, integers → `p_integer`, floats → `p_double`, strings → `p_string`, length-3 number arrays → `p_vector_3d`; other JS values get JSON-stringified into a `p_string`. |
...
- **`bakeSpaceTransform: true`** only pre-multiplies the axis matrix into **vertex positions + normals**. Object `Lcl` transforms, cluster matrices, animation curves, and light/camera transforms are NOT baked (Blender bakes them all). The flag is only safe for single-mesh-at-origin scenes; leave it `false` otherwise — modern importers (Unity, Unreal, Blender, Maya) honour `GlobalSettings.UpAxis` so axis conversion happens on import.
- **`light.target.position`** for `DirectionalLight` / `SpotLight` round-trips via a synthetic `LookAtProperty` Model + OP connection (Blender writes a converted rotation matrix instead — three.js's `FBXLoader` only supports the `LookAtProperty` path).
- **Output is always binary FBX** (`Kaydara FBX Binary` magic). ASCII FBX is not implemented — neither is it in Blender's exporter.
...
Mirrors the architecture of Blender's `export_fbx_bin.py` so behaviour stays aligned with the reference exporter. 23 bugs were surfaced and fixed during deep audits + Blender oracle byte-comparison while building this; the commit log on `master` captures the milestone trail.
...
446 unit tests covering the binary layer, scene collection, every Object kind (Mesh / Material / Texture / Skin / Animation / Morph / Light / Camera), axis/unit conversion, the public-options surface, and structural-oracle diffs against Blender's reference `bl_cube.fbx` / `bl_cube_skinned.fbx`. The oracle tests self-skip when the Blender source corpus isn't available locally.

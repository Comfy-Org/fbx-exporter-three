 ...
 *
 * Mirrors `export_fbx_bin.py: fbx_header_elements / fbx_documents_elements /
...
export function writeHeaderSection({ root, settings, sceneName = 'Scene' }) {
  ...
  // Time stamp — fixed for determinism (Blender does the same; the file's
  // real time is overwritten by the time-hack later in encodeBinaryFBX).
  ...
  // SceneInfo — Maya/Unity reject files without it
  ...
  // "Original" + "LastSaved" compound props — Blender writes them flat-keyed.
  ...
  // Top-level placeholders — get rewritten by encodeBinary's time-hack
  ...
  elemEmpty(root, 'References'); // intentionally empty
}
...
function writeDocuments(root, sceneName) {
  ...
  // Document UID is derived from scene name so re-exports stay deterministic.
...
// A tiny stable hash, separate from UidRegistry to avoid pulling the registry
// into a single-call site. Result is positive, fits in int63.

 ...
 *
 * Mirrors Blender's `io_scene_fbx/encode_bin.py: FBXElem`.
 *
...
export class FBXElem {
  ...
  constructor(id: string) {
    ...
    // FBX writes the id length into a single uint8 — must check the encoded
    // BYTE length, not the JS character length. Multibyte UTF-8 ids
    // (e.g. CJK characters) easily exceed 255 bytes while staying under 255
    // characters and would otherwise overflow the field silently.
    ...
    // Single-byte type tags, one per prop.
    this.propsType = [];
    // Per-prop encoded payload (NO leading type tag).
    this.propsData = [];

    this.elems = [];

    // Filled by _calcOffsets.
    this._endOffset = -1;
    this._propsLength = -1;
  }
  ...
  // ---------- offset calculation & write (two-pass) ----------
  ...
  _calcOffsets(offset, isLast, ctx) {
    offset += ctx.metaSize;          // (end_offset, prop_count, props_length) header
    offset += 1 + this._idBytes.length; // 1 byte id length + id bytes

    let propsLength = 0;
    for (let i = 0; i < this.propsData.length; i++) {
      propsLength += 1 + this.propsData[i].byteLength; // 1 byte type tag + payload
    }
  ...
  _write(bw, isLast, ctx) {
    ...
    // meta header: end_offset, prop_count, props_length
    if (ctx.use64) {

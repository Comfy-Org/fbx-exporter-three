/**
 * Convenience builders that mirror Blender's `fbx_utils.py` helpers — the
 * `elem_data_single_*`, `elem_properties` and `elem_props_*` families.
 *
...
// ---------------------------------------------------------------------------
// elem_data_single_* — create a child node carrying exactly one scalar/array prop
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Properties70 — the FBX "P" node grammar.
//
// Each "P" node is a string-tagged record:
//   P [name] [type] [subtype/label] [flags] <value(s)...>
// `type` and `subtype` are the FBX metatype strings ("Lcl Translation", "double",
// "ColorRGB", ...); `flags` is "" or "A" / "A+" / "A+U" depending on animatable
// + animated + custom flags.
//
// We map a short "ptype" string (`p_double`, `p_lcl_translation`, ...) to the
// canonical (typeStr, subtypeStr, adder list). Mirrors FBX_PROPERTIES_DEFINITIONS.
// ---------------------------------------------------------------------------

// adder names — referenced from PTYPES table below
...
function writeOneProp(propsElem, ptypeDef, name, value, flags) {
  ...
  p.addString(name);
  p.addString(ptypeDef[0]);    // type
  p.addString(ptypeDef[1]);    // subtype/label
  p.addString(flags);
  const adders = ptypeDef.slice(2);
  if (adders.length === 1) {
    if (ptypeDef[0] === 'object') {
      // p_object has zero values
    } else {
      p[adders[0]](value);
    }
...
// ---------------------------------------------------------------------------
// Template-aware property writing
//
// A "template" object describes what default values to expect (from the
// Definitions section). When a per-instance property matches the template's
// default, we skip writing it.
//
// templateInit  → produce a working copy of the template's prop dict
// templateSet   → write prop unless it equals the template default
// templateFinalize → write any template properties that weren't yet emitted (for
//                    cases where a different subtype needs them)
//
// Mirrors Blender's elem_props_template_init/set/finalize.
// ---------------------------------------------------------------------------
...
/**
 * Write a per-instance P record. Unlike Blender's elem_props_template_set
 * we do NOT skip on template-match: three.js FBXLoader reads per-instance
...
// No-op finalizer kept as a call-site stub so per-instance Properties70
// blocks have a uniform termination point.
